...
cont_osnr=my_plot_range;
cont_piny=my_plot_range;

for p_iny=p_dbm_range
%%%%%%%%%%%%%%%%% Field parameters
Nsymb = 512; % number of symbols - 512
Nt = 32; % points x symbol
Nch = 1; % number of channels
%%%%%%%%%%%%%%%% Pulse parameters
phi = 0.2*pi; % average cumulated nonlinear phase
exratio = fiberexratio; % extinction ratio [dB] 
lam = 1550; % central wavelength [nm]
spac = 0.4; % channel spacing [nm]
symbrate = 10.7; % baudrate [Gbaud] - 10.7
duty = 1; % duty cycle
...
Pdbm=p_iny; %Potencia inyectada en dBm
Pavg=10^(Pdbm/10);%Potencia en mW
...
%%%%%%%%%% Optical link
ampli.f = 5.5; %Ruido amplificador [dB] 5.5 defino ruido
for k=1:Nspan
    fiber(tx,'g-sx')
    fiber(comp,'g-sx')
    ampliflat(Gerbio,'gain',ampli) %-añadido ruido
end
...
cont_osnr(p_iny+col)=osnr;
cont_piny(p_iny+col)=p_iny;
saveas(figure(3),sprintf('Eye%d.png',p_iny)); % will create FIG1, FIG2,...
close(3)
...
saveas(figure(4),sprintf('BERvsOSNR%d.png',p_iny)); % will create FIG1, FIG2,...
close(4)
end

figure(5)
plot (cont_piny(my_plot_range),cont_osnr(my_plot_range));
grid on
title('OSNR vs P_inyectada');
xlabel('OSNR')
ylabel('P_inyectada')
saveas(figure(5),sprintf('exratiovsOSNR.png')); % will create FIG1, FIG2,...
fprintf('\n\n=========== Total OSNR ===========\n\n');
cont_osnr(my_plot_range)